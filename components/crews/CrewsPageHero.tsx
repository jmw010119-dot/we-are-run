...
type CrewsPageHeroProps = {
  query: string;
  onQueryChange: (query: string) => void;
};

export function CrewsPageHero({ query, onQueryChange }: CrewsPageHeroProps) {
  return (
    <Section spacing="lg" className="overflow-hidden pt-16 md:pt-20">
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_0.75fr] lg:items-end">
        <div>
          <p className="text-xs font-black tracking-[0.22em] text-run-lime">RUNNING CREW</p>
          ...
          <p className="mt-6 max-w-2xl break-keep text-base font-semibold leading-7 text-run-muted md:text-lg">
            지역별 정기런과 번개런을 운영하는 러닝 크루를 한눈에 확인하세요.
          </p>
        </div>
        <SearchInput
          aria-label="러닝 크루 검색"
          placeholder="지역, 크루명, 러닝 유형을 검색하세요"
          value={query}
